      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-1">
          {navLinks.map((link) => (
            ...
            >
              <button className="flex items-center gap-1 text-sm text-white/60 hover:text-white transition px-3 py-2 rounded-lg hover:bg-white/5">
                ...
                <div className="absolute top-full left-0 mt-1 w-60 bg-surface-2 border border-border rounded-xl p-2 shadow-2xl shadow-black/20">
                  {link.sub.map((s) => (
                    <Link
                      key={s.href}
                      to={s.href}
                      className="block text-sm text-white/60 hover:text-accent hover:bg-white/5 px-3 py-2.5 rounded-lg transition"
                    >
                      {s.label}
                    </Link>
                  ))}
          ...
          <Link to="/contact" className="text-sm text-white/60 hover:text-white transition px-3 py-2">
            {t('nav.contact')}
          </Link>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <div
            ...
            onMouseLeave={() => setDropdown(null)}
          >
            <button className="flex items-center gap-1.5 text-sm text-white/60 hover:text-white transition px-2.5 py-2 rounded-lg hover:bg-white/5">
              <Globe size={15} />
              ...
              <div className="absolute top-full right-0 mt-1 w-44 bg-surface-2 border border-border rounded-xl p-2 shadow-2xl shadow-black/20">
                {languages.map((lang) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 text-sm px-3 py-2.5 rounded-lg transition ${lang.code === i18n.language ? 'text-accent bg-white/5' : 'text-white/60 hover:text-white hover:bg-white/5'}`}
                  >
          ...
          <a href="https://bo.matgaming.net" target="_blank" rel="noopener noreferrer" className="text-sm text-white/60 hover:text-white transition px-3 py-2 rounded-lg border border-white/15 hover:border-white/30">
            Backoffice
          </a>
          <a href="https://matslot.com" target="_blank" rel="noopener noreferrer" className="bg-accent hover:bg-accent-light text-brand-dark px-5 py-2 rounded-full text-sm font-semibold transition">
            Platform
          </a>
        </div>
